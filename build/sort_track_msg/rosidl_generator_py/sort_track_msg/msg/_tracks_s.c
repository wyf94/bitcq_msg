...
bool sort_track_msg__msg__tracks__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("sort_track_msg.msg._tracks.Tracks", full_classname_dest, 33) == 0);
  }
  sort_track_msg__msg__Tracks * ros_message = _ros_message;
  {  // header
  ...
  {  // bounding_boxes
    ...
    if (!sort_track_msg__msg__BoundingBox__Sequence__init(&(ros_message->bounding_boxes), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create sort_track_msg__msg__BoundingBox__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    sort_track_msg__msg__BoundingBox * dest = ros_message->bounding_boxes.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!sort_track_msg__msg__bounding_box__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * sort_track_msg__msg__tracks__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("sort_track_msg.msg._tracks");
  ...
  {  // bounding_boxes
    ...
    sort_track_msg__msg__BoundingBox * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->bounding_boxes.data[i]);
      PyObject * pyitem = sort_track_msg__msg__bounding_box__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
